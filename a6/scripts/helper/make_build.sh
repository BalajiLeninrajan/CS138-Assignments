...
# find the correct .cc and .h files
if [ $1 = "1" ] ; then
    COMPILE_FILES="InputFiles/* HashTable.cc HashTable.h a6p1*"
elif [ $1 = "2" ] ; then
    COMPILE_FILES="InputFiles/* HashTable.cc HashTable.h a6p1.* a6p2*"
elif [ $1 = "3" ] ; then
    COMPILE_FILES="InputFiles/* HashTable.cc HashTable.h a6p1.* a6p2.* a6p3*"
fi
# remove the old build directory
rm -rf $OUTPUT_MAKE/*
# copy in source files
cp -f $COMPILE_FILES $OUTPUT_MAKE/ > /dev/null 2>&1
