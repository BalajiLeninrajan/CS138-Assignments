...
TEST(StackForm, SubsequentChunksShouldNotHaveUnusedSlots)
{
    Stack s;
    initStack(1, s);
    s.topElt = 0;
    ...
    s.firstChunk->next = createNewNodeChunk(1);
    s.firstChunk->next->val[0] = "alpha";
    s.firstChunk->next->next = createNewNodeChunk(1);
    s.firstChunk->next->next->val[0] = "gamma";
    EXPECT_FALSE(isEmpty(s));
    EXPECT_TRUE(isValidStack(s));
    // wellformed so far
    // now we break it
    s.firstChunk->next->next->val[0] = UNUSED_SLOT;
    EXPECT_DEATH(isValidStack(s), "");
    delete[] s.firstChunk->next->next->val;
    delete s.firstChunk->next->next;
    delete[] s.firstChunk->next->val;
    delete s.firstChunk->next;
    delete[] s.firstChunk->val;
    delete s.firstChunk;
...
TEST(EndToEnd, GivenMoreElementsThanChuckSizeExpectMultipleChunks)
{
    Stack s;
    initStack(1, s);
    push("First", s);
    ...
    EXPECT_TRUE(s.firstChunk && s.firstChunk->next);
...
TEST(EndToEnd, GivenEmptyStackEmptyArrayExpectedFromToString)
{
    Stack s;
    initStack(3, s);
...
TEST(EndToEnd, GivenTwoPushedElementsToDifferentChunksCombinationOfTheseTwoIsExpectedFromToString)
{
    Stack s;
    initStack(1, s);
    push("alpha", s);
...
TEST(EndToEnd, PopIsInvokedOnEmptyStackExpectDeath)
{
    Stack s;
    initStack(3, s);
    EXPECT_DEATH(pop(s), "");
...
TEST(EndToEnd, GivenOnePushedElementsWhenPopIsInvokedExpectEmptyStack)
{
    Stack s;
    initStack(3, s);
    push("alpha", s);
...
TEST(EndToEnd, GivenMoreElementsThanChuckSizeWhenPopIsInvokedExpectOneLessNode)
{
    Stack s;
    initStack(1, s);
    push("First", s);
...
TEST(EndToEnd, GivenEmptyStackTheSizeOfZeroIsExpected)
{
    Stack s;
    initStack(3, s);
    EXPECT_EQ(0, size(s));
...
TEST(EndToEnd, GivenStackWithMultipleChunksExpectAllChunksToBeCounted)
{
    Stack s;
    initStack(1, s);
...
TEST(EndToEnd, GivenTwoPushedElementsInDifferentChunksWhenSwapIsInvokedSwappedItemsExpectedFromToString)
{
    Stack s;
    initStack(1, s);
...
TEST(EndToEnd, GivenAnElementAddedWhenNukeIsInvokedAnEmptyStackIsExpected)
{
    Stack s;
    initStack(3, s);
