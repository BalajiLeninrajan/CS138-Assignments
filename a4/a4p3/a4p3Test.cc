...
TEST(sizeNew, givenTwoElementSBLExpectSizeIsTwo)
{
    SBL sbl;
    sbl.q.first = sbl.root = new SBLnode{"test", nullptr, nullptr, nullptr};
    sbl.q.first->next = sbl.q.last = sbl.root->left = new SBLnode{"test", nullptr, nullptr, nullptr};
    sbl.numElts = 2;
    EXPECT_EQ(SBL_size(sbl), 2);
    delete sbl.root->left;
    delete sbl.root;
}

TEST(hasNew, givenSevenElementSBLExpectItHasAllElements)
{
    SBL sbl;
    SBL_init(sbl);
    SBL_arrive(sbl, "f");
    ...
    SBL_arrive(sbl, "g");
    SBL_arrive(sbl, "i");
    EXPECT_TRUE(SBL_has(sbl, "f"));
    EXPECT_TRUE(SBL_has(sbl, "b"));
    EXPECT_TRUE(SBL_has(sbl, "h"));
    EXPECT_TRUE(SBL_has(sbl, "a"));
    EXPECT_TRUE(SBL_has(sbl, "d"));
    EXPECT_TRUE(SBL_has(sbl, "c"));
    EXPECT_TRUE(SBL_has(sbl, "e"));
    EXPECT_TRUE(SBL_has(sbl, "g"));
    EXPECT_TRUE(SBL_has(sbl, "i"));
    SBL_nuke(sbl);
}

TEST(removeNew, givenSevenElementSBLExpectRemoveCorrectContents)
{
    SBL sbl;
    SBL_init(sbl);
    SBL_arrive(sbl, "f");
    ...
    SBL_leave(sbl);
    SBL_leave(sbl);
    EXPECT_FALSE(SBL_has(sbl, "f"));
    EXPECT_FALSE(SBL_has(sbl, "b"));
    EXPECT_FALSE(SBL_has(sbl, "h"));
    EXPECT_TRUE(SBL_has(sbl, "a"));
    EXPECT_TRUE(SBL_has(sbl, "d"));
    EXPECT_TRUE(SBL_has(sbl, "c"));
    EXPECT_TRUE(SBL_has(sbl, "e"));
...
TEST(removeNew, givenFourElementSBLExpectRemoveCorrectContents)
{
    SBL sbl;
    SBL_init(sbl);
    SBL_arrive(sbl, "d");
    ...
    SBL_leave(sbl);
    SBL_leave(sbl);
    EXPECT_FALSE(SBL_has(sbl, "d"));
    EXPECT_FALSE(SBL_has(sbl, "z"));
    EXPECT_FALSE(SBL_has(sbl, "b"));
    EXPECT_FALSE(SBL_has(sbl, "a"));
    EXPECT_TRUE(SBL_has(sbl, "w"));
    EXPECT_TRUE(SBL_has(sbl, "x"));
    EXPECT_TRUE(SBL_has(sbl, "y"));
    SBL_leave(sbl);
    SBL_leave(sbl);
    SBL_leave(sbl);
    EXPECT_EQ(sbl.numElts, 0);
